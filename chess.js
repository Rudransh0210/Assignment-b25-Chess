...
const BLACK_TILE_COLOR = "#b29212";
const HIGHLIGHT_COLOR = "#1297b2";
const WHITE = 0;
const BLACK = 1;
...
const piecesCharacters = {0: '♙',1: '♘',2: '♗',3: '♖',4: '♕',5: '♔'};
...
function onLoad() {
    chessCanvas = document.getElementById("canvas");
...
function checkPossiblePlays() {
    ...
    drawTile(xx, yy, HIGHLIGHT_COLOR);

    board.resetValidMoves();

    if (tile.pieceType === PAWN) checkPossiblePlaysPawn(xx, yy);
    else if (tile.pieceType === KNIGHT) checkPossiblePlaysKnight(xx, yy);
    else if (tile.pieceType === BISHOP) checkPossiblePlaysBishop(xx, yy);
    else if (tile.pieceType === ROOK) checkPossiblePlaysRook(xx, yy);
    else if (tile.pieceType === QUEEN) checkPossiblePlaysQueen(xx, yy);
    else if (tile.pieceType === KING) checkPossiblePlaysKing(xx, yy);
}

function checkPossiblePlaysPawn(xx, yy) {
    let direction;

    if (currentTeam === WHITE) direction = -1;
    else direction = 1;

    if (yy+direction < 0 || yy+direction > BOARD_HEIGHT-1) return;

    checkPossibleMove(xx, yy+direction);

    if (yy === 1 || yy === 6) {
        checkPossibleMove(xx, yy+2*direction);
    }

    if (xx-1 >= 0) checkPossibleCapture(xx-1, yy+direction);

    if (xx+1 <= BOARD_WIDTH-1) checkPossibleCapture(xx+1, yy+direction);
}

function checkPossiblePlaysKnight(xx, yy) {
    if (xx-2 >= 0) {
        if (yy-1 >= 0) checkPossiblePlay(xx-2, yy-1);

        if (yy+1 <= BOARD_HEIGHT-1) checkPossiblePlay(xx-2, yy+1);
    }

    if (xx-1 >= 0) {
        if (yy-2 >= 0) checkPossiblePlay(xx-1, yy-2);

        if (yy+2 <= BOARD_HEIGHT-1) checkPossiblePlay(xx-1, yy+2);
    }

    if (xx+1 <= BOARD_WIDTH-1) {
        if (yy-2 >= 0) checkPossiblePlay(xx+1, yy-2);

        if (yy+2 <= BOARD_HEIGHT-1) checkPossiblePlay(xx+1, yy+2);
    }

    if (xx+2 <= BOARD_WIDTH-1) {
        if (yy-1 >= 0) checkPossiblePlay(xx+2, yy-1);
...
function checkPossiblePlaysRook(xx, yy) {
    for (let i = 1; yy-i >= 0; i++) {
        if (checkPossiblePlay(xx, yy-i)) break;
    }

    for (let i = 1; xx+i <= BOARD_WIDTH-1; i++) {
        if (checkPossiblePlay(xx+i, yy)) break;
    }

    for (let i = 1; yy+i <= BOARD_HEIGHT-1; i++) {
        if (checkPossiblePlay(xx, yy+i)) break;
    }

    for (let i = 1; xx-i >= 0; i++) {
        if (checkPossiblePlay(xx-i, yy)) break;
    }
}

function checkPossiblePlaysBishop(xx, yy) {
    for (let i = 1; xx+i <= BOARD_WIDTH-1 && yy-i >= 0; i++) {
        if (checkPossiblePlay(xx+i, yy-i)) break;
    }

    for (let i = 1; xx+i <= BOARD_WIDTH-1 && yy+i <= BOARD_HEIGHT-1; i++) {
        if (checkPossiblePlay(xx+i, yy+i)) break;
    }

    for (let i = 1; xx-i >= 0 && yy+i <= BOARD_HEIGHT-1; i++) {
        if (checkPossiblePlay(xx-i, yy+i)) break;
    }

    for (let i = 1; xx-i >= 0 && yy-i >= 0; i++) {
        if (checkPossiblePlay(xx-i, yy-i)) break;
    }
}

function checkPossiblePlaysQueen(xx, yy) {
    checkPossiblePlaysBishop(xx, yy);
    checkPossiblePlaysRook(xx, yy);
}

function checkPossiblePlaysKing(xx, yy) {
    for (let i = -1; i <= 1; i++) {
        if (yy+i < 0 || yy+i > BOARD_HEIGHT-1) continue;

        for (let j = -1; j <= 1; j++) {
            if (xx+j < 0 || xx+j > BOARD_WIDTH-1) continue;
            if (i == 0 && j == 0) continue;

            checkPossiblePlay(xx+j, yy+i);
        }
    }
}
...
function moveSelectedPiece(x, y) { 
    board.tiles[y][x].pieceType = board.tiles[yy][xx].pieceType;
    board.tiles[y][x].team = board.tiles[yy][xx].team;

    board.tiles[yy][xx].pieceType = EMPTY;
    board.tiles[yy][xx].team = EMPTY;
...
class Board {
    constructor() {
        ...
        this.tiles.push([new Tile(ROOK, BLACK),new Tile(KNIGHT, BLACK),new Tile(BISHOP, BLACK),new Tile(QUEEN, BLACK),new Tile(KING, BLACK),new Tile(BISHOP, BLACK),new Tile(KNIGHT, BLACK),new Tile(ROOK, BLACK)]);

        this.tiles.push([new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK),new Tile(PAWN, BLACK)]);

        for (let i = 0; i < 4; i++) {
            this.tiles.push([new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),new Tile(EMPTY, EMPTY),]);
        }

        this.tiles.push([new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE),new Tile(PAWN, WHITE)]);

        this.tiles.push([new Tile(ROOK, WHITE),new Tile(KNIGHT, WHITE),new Tile(BISHOP, WHITE),new Tile(QUEEN, WHITE),new Tile(KING, WHITE),new Tile(BISHOP, WHITE),new Tile(KNIGHT, WHITE),new Tile(ROOK, WHITE)]);

        this.validMoves = [];
        for (let i = 0; i < BOARD_HEIGHT; i++) {
            this.validMoves.push([INVALID,INVALID,INVALID,INVALID,INVALID,INVALID,INVALID,INVALID]);
        }
    }
